...
export default class MyProjectsToReviewRoute extends Route {
  ...
  async model() {
    ...
    let user = await this.currentUser.get('user');
    let toReviewProjectsRaw = await fetch(`/users/${user.id}/projects?projectState=to-review`);
    let toReviewProjectsIds = await toReviewProjectsRaw.json();
    return this.store.findAll('project', { ids: toReviewProjectsIds });s
  }
}
